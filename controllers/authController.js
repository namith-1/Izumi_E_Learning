...
exports.signup = async (req, res) => {
  ...
  try {
    // Determine if student or instructor based on role (default to student for form submissions)
    ...
    if (isInstructor) {
      ...
      // If this is a form submission (not AJAX), redirect to student home
...
exports.loginStudent = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      // If this is a form submission (not AJAX), redirect with error
      if (req.headers['content-type'] && req.headers['content-type'].includes('application/x-www-form-urlencoded')) {
        return res.redirect('/student/login?error=invalid_credentials');
      }
    ...
    if (match) {
      ...
      // If this is a form submission, redirect to student home
      if (req.headers['content-type'] && req.headers['content-type'].includes('application/x-www-form-urlencoded')) {
        return res.redirect('/dashboard');
      }
    ...
    console.error("Login error:", error);
    if (req.headers['content-type'] && req.headers['content-type'].includes('application/x-www-form-urlencoded')) {
      return res.redirect('/student/login?error=server_error');
    }
    return res.status(500).json({ message: "Error logging in: " + error.message });
  }
};
