...
exports.me = (req, res) => {
    if (req.session.user) {
        ...
        if (req.session.user.id === MOCK_ADMIN_ID && req.session.user.role === 'admin') {
        ...
        if (!Model) {
            return res.status(404).json({ user: null, message: 'Invalid session role.' });
        }
...
exports.updateProfile = async (req, res) => {
    try {
        ...
        if (userId === MOCK_ADMIN_ID && req.session.user.role === 'admin') {
             return res.status(403).json({ message: 'Cannot update profile for administrative account.' });
        }
        // --- END Block ---
        ...
        res.json({ message: 'Profile updated successfully', user: { id: user._id, role: req.session.user.role, name: user.name, email: user.email } });
