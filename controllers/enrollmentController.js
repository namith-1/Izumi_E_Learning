...
const checkCourseCompletion = async (courseId, enrollment) => {
    ...
    const allModulesMap = { ...course.modules, [rootModuleId]: course.rootModule };
    ...
    const nonRootContentModules = allModulesList.filter(m => m.id !== rootModuleId && m.id !== courseObjectId);
    ...
    const completedContentModules = nonRootContentModules
    ...
    const allQuizzesCompleted = quizModules.every(quiz => completedModuleIds.includes(quiz.id));
...
exports.getMyEnrolledCourses = async (req, res) => {
    try {
        ...
        const enrollments = await Enrollment.find({ studentId: req.session.user.id })
        ...
        const enrolledCoursesData = enrollments.map(enrollment => {
            ...
            const nonRootContentModules = allModulesList.filter(m => m.id !== rootModuleId && m.id !== course._id.toString());
            const totalContentModules = nonRootContentModules.length;

            const completedModulesCount = enrollment.modules_status
                .filter(s => s.completed)
                .map(s => s.moduleId)
                .filter(id => nonRootContentModules.some(m => m.id === id)) // Ensure completed ID is a real content module
                .length;
            // ---------------------------------
