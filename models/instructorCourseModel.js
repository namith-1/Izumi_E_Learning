...
const CourseModel = {
    ...
    getAllCourses: async () => {
        try{
        return await Course.find({}, '_id title instructor_id subject').sort({ subject: 1, title: 1 }).lean().exec();
        }catch(error){
            throw error;
        }
    },
    ...
    getInstructorCourses: async (instructorId) => {
        try{
            console.log( Course.find({ instructor_id: instructorId }, '_id title'));
        return await Course.find({ instructor_id: instructorId }, '_id title').lean().exec();
        }catch(error){
            throw error;
        }
    },
    ...
    findCourseByIdAndInstructor: async (courseId, instructorId) => {
        try {
            ...
            const course = await Course.findOne({ _id: courseId, instructor_id: instructorId }, 'id').lean().exec();
            return course;
        } catch (error) {
            throw error;
        }
            ...
            return Course.aggregate([
                ...
                },
                {
                    $project: {
                        ...
                        enrolled_count: {
                            $ifNull: [
                                { $arrayElemAt: ['$stats.enrolled_count', 0] },
                                0
                            ]
                        },
                        price: {
                            $ifNull: [
                                { $arrayElemAt: ['$stats.price', 0] },
                                0
                            ]
                        },
                        revenue: {
