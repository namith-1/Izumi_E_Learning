...
// 11. Instructor Revenue Analytics - Revenue data for instructor's courses
exports.getInstructorRevenueAnalytics = async (req, res) => {
    try {
        let instructorId = new mongoose.Types.ObjectId(req.session.user.id);
        ...
        if (courseIds.length === 0) {
            return res.json([]); // No courses, return empty
        }

        // 1. Aggregate enrollments by Date AND Course ID for this instructor's courses
        const instructorRevenueTimes = await EnrollmentAnalytics.aggregate([
            {
                $match: {
        ...
        if (instructorRevenueTimes.length === 0) {
            return res.json([]);
        }

        // 2. Extract unique course IDs and fetch their titles
        const uniqueCourseIds = [...new Set(instructorRevenueTimes.map(item => item.courseId.toString()))];
        const courseTitles = await Course.find({ _id: { $in: uniqueCourseIds } }).select("title");

        // 3. Create a dictionary for course titles
        const courseTitleMap = {};
        courseTitles.forEach(course => {
            courseTitleMap[course._id.toString()] = course.title;
        });

        // 4. Map over the analytics results and inject the courseName
        const enrichedAnalytics = instructorRevenueTimes.map(record => ({
            date: record.date,
            courseId: record.courseId,
            courseName: courseTitleMap[record.courseId.toString()] || "Unknown Course",
            dayWisePrice: record.dayWisePrice,
        ...
        console.error("Instructor Revenue Analytics Error:", err);
        res.status(500).json({ error: err.message });
    }
};
