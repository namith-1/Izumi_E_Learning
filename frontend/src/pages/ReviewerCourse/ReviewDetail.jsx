...
const ReviewDetail = () => {
  ...
  const handleAction = async (action) => {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}/review/course/${courseId}/${action}`,
        {
          method: "POST",
  ...
  if (loading)
    return (
      <div style={{ padding: 24, color: "#9ca3af" }}>Loading course...</div>
    );
  if (!course)
    return (
  ...
  return (
    <div>
      ...
      >
        <div>
          ...
          <p style={{ margin: 0, color: "#6b7280" }}>
            {course.subject} &middot; {course.teacherId?.name || "Unknown"}
          </p>
        </div>
      ...
      <div style={{ marginBottom: 16 }}>
        <h4 style={{ marginBottom: 8, color: "#111827" }}>
          Course Structure ({modulesList.length} items)
        </h4>
            ...
            <tbody>
              {modulesList.map((mod) => (
                <tr key={mod.id}>
                  <td>
                    <span
                      className={`status-badge ${mod.type === "quiz" ? "completed" : "in-progress"}`}
                    >
                      {typeLabel(mod.type)}
                    </span>
                  </td>
      ...
      {course.reviewNotes?.length > 0 && (
        ...
        >
          <h4 style={{ margin: "0 0 8px", color: "#111827" }}>
            Review Notes ({course.reviewNotes.length})
          </h4>
