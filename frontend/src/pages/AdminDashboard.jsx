...
const SearchBar = ({ value, onChange, placeholder }) => (
  <div
    className="search-input-group mb-4"
    style={{
      maxWidth: "400px",
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    if (activeTab === "reviewers") {
      ...
      fetch(`${API_BASE_URL}/admin/reviewers`, { credentials: "include" })
        .then((r) => r.json())
        .then((data) => {
  ...
  const handleSaveCourse = (id) => {
    dispatch(updateCourseAdmin({ id, data: courseEditData })).then(() =>
  ...
  const LookupResults = () => {
    if (lookupLoading)
      return (
        <div className="p-4 text-center">
          <Loader2 className="animate-spin inline" /> Loading details...
        </div>
      );
    if (lookupError)
      return (
    ...
    return (
      <div
        id="lookup-results"
        className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-lg shadow-sm"
      >
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-bold text-blue-900">
            Details for {lookupType === "teacher" ? "Instructor" : "Student"}:{" "}
            {lookupResult.teacher?.name || lookupResult.student?.name}
          </h3>
          <button
            onClick={() => dispatch(clearLookup())}
            ...
            <tbody>
              {lookupResult.enrollments.map((e) => (
                <tr key={e._id}>
                  ...
                  <td>
                    <span className={`status-badge ${e.completionStatus}`}>
                      {e.completionStatus}
                    </span>
                  </td>
                  <td>{e.progress}%</td>
              ...
              {lookupResult.enrollments.length === 0 && (
                <tr>
  ...
  const renderInstructors = () => {
    const instructorStats = [
      ...
      },
      {
        ...
        value: teachers.filter((t) => (t.courseCount || 0) > 0).length,
        icon: BookOpen,
        color: "green",
      },
      {
        label: "Total Students Taught",
        value: teachers.reduce((acc, t) => acc + (t.totalStudents || 0), 0),
        icon: Users,
        color: "purple",
      },
    ];
    ...
    return (
      <div>
        ...
        <div className="flex justify-between items-end mb-4">
          <h3 className="tab-section-title mb-0">
            Instructors List ({filteredTeachers.length})
          </h3>
        </div>
        <SearchBar
              ...
              {filteredTeachers.map((t) => (
                <tr key={t._id}>
                  ...
                  <td className="flex gap-2">
                    <button
                      onClick={() => handleLookup("teacher", t.email)}
  ...
  const renderStudents = () => {
    const activeLearners = new Set(
      enrollments.map((e) => e.studentId?._id || e.studentId),
    ).size;
    const completionRate =
      enrollments.length > 0
        ? Math.round(
            (enrollments.filter((e) => e.completionStatus === "completed")
              .length /
              enrollments.length) *
              100,
          )
            ...
            <tbody>
              {filteredStudents.map((s) => (
                <tr key={s._id}>
                  ...
                  <td className="flex gap-2">
                    <button
                      onClick={() => handleLookup("student", s.email)}
                      className="btn-action-edit"
                      title="View Progress"
                    >
  ...
  const renderCourses = () => {
    const totalEnrollments = courses.reduce(
      (acc, c) => acc + (c.totalStudentsRegistered || 0),
      0,
    );
    const avgRating =
      courses.length > 0
    ...
    return (
      <div>
        ...
        <SearchBar
          ...
          placeholder="Search Title, Subject or Instructor..."
        />
                ...
                <tr key={c._id}>
                  {isEditingCourse === c._id ? (
                    <>
                      ...
                      <td>
                        <span
                          className={`status-badge ${c.approvalStatus || "draft"}`}
                        >
                      ...
                      <td className="flex gap-1">
                        <button
                          onClick={() => handleSaveCourse(c._id)}
                          className="btn-action-save"
                        >
                          <Save size={16} />
                        </button>
                        <button
                          onClick={() => setIsEditingCourse(null)}
  ...
  const handleCreateReviewer = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setReviewerMsg({
          text: `Reviewer "${data.reviewer.name}" created!`,
          type: "success",
        });
        setReviewerForm({
          name: "",
        ...
        // Refresh list
        const listRes = await fetch(`${API_BASE_URL}/admin/reviewers`, {
          credentials: "include",
        });
        if (listRes.ok) setReviewers(await listRes.json());
      } else {
        setReviewerMsg({
          text: data.message || "Failed to create reviewer.",
          type: "error",
        });
      }
    } catch (err) {
  ...
  const handleDeleteReviewer = async (id) => {
    ...
    try {
      await fetch(`${API_BASE_URL}/admin/users/reviewer/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  const renderReviewers = () => (
    <div>
      ...
      >
        ...
        {reviewerMsg.text && (
          <div
            style={{
              ...
              fontSize: 13,
              background:
                reviewerMsg.type === "success" ? "#d1fae5" : "#fee2e2",
        ...
        >
          <input
            ...
            onChange={(e) =>
              setReviewerForm({ ...reviewerForm, name: e.target.value })
            }
            style={{
              padding: "10px 14px",
      ...
      {/* Reviewers List */}
      <h3 className="tab-section-title">
        Reviewer Accounts ({reviewers.length})
      </h3>
      {reviewerLoading ? (
        <div className="p-4 text-center">
            ...
            <tbody>
              {reviewers.map((r) => (
                <tr key={r._id}>
                  ...
                  <td>
                    <button
                      onClick={() => handleDeleteReviewer(r._id)}
                      className="btn-action-delete"
                    >
                      <Trash2 size={16} />
                    </button>
              ...
              {reviewers.length === 0 && (
                <tr>
                  ...
                  >
                    No reviewers yet. Create one above.
                  </td>
                </tr>
              )}
            </tbody>
  ...
  return (
    <div className="admin-dash-layout student-dash-layout">
      <header className="student-navbar">
        <div className="nav-brand text-green-500">
          <BarChart3 size={24} /> <span>Admin Panel</span>
        </div>
        <nav className="nav-links">
          ...
          <button
            ...
            className={`nav-link-item ${activeTab === "revenue" ? "active" : ""}`}
          >
            <DollarSign size={18} /> Revenue
          </button>
          <button
            onClick={() => setActiveTab("instructors")}
            className={`nav-link-item ${activeTab === "instructors" ? "active" : ""}`}
          >
            <Users size={18} /> Instructors
          </button>
          <button
            onClick={() => setActiveTab("students")}
            className={`nav-link-item ${activeTab === "students" ? "active" : ""}`}
          >
            <Users size={18} /> Students
          </button>
          <button
            onClick={() => setActiveTab("courses")}
            className={`nav-link-item ${activeTab === "courses" ? "active" : ""}`}
          >
            <BookOpen size={18} /> Courses
          </button>
          <button
            onClick={() => setActiveTab("reviewers")}
            className={`nav-link-item ${activeTab === "reviewers" ? "active" : ""}`}
          >
            <Shield size={18} /> Reviewers
          </button>
        </nav>
        <div className="nav-user-info">
          <ProfileDropdown user={user} currentPath="/admin" />
        </div>
      </header>

      <main className="student-main-content">
        {activeTab === "analytics" && <AnalyticsDashboard />}
        {activeTab === "revenue" && <RevenueAnalyticsDashboard />}
